# Deployment link :


https://tai-music.herokuapp.com/





# Sources intéressantes : 
## Pyo : un module python pour le traitement de fichiers audio
http://www.augmented-instruments.net/_media/pyo_as_python_dsp_toolbox.pdf

## PyMedia : un module python pour lire des fichiers multimédias
http://pymedia.org/tut/play_wav.html

## webaudio : un module JavaScript pour lire, synchroniser des fichiers audio
https://web.dev/webaudio-intro/

## HTML5 avec JS pour les fichiers audios : 
https://yard.onl/sitelycee/cours/js/_Js.html?Laudio.html

## Fonctions asynchrones en JS : 
https://developer.mozilla.org/fr/docs/Learn/JavaScript/Asynchronous/Introducing



